...
export const postLogin = async (req, res, next) => {
    try {
        ...
        res.cookie('token', accessToken, { httpOnly: true, sameSite: 'lax', path: '/', secure: process.env.NODE_ENV === 'production', maxAge: 15 * 60 * 1000 })
        res.cookie('refreshToken', refreshToken, { httpOnly: true, sameSite: 'lax', path: '/', secure: process.env.NODE_ENV === 'production', maxAge: 7 * 24 * 60 * 60 * 1000 })
        ...
        try {
            await logEvent({ userId: null, event: 'login', success: false, ip: req.ip, userAgent: req.headers['user-agent'] })
        } catch { }
        next(error)
    }
}
...
export const postRegister = async (req, res, next) => {
    try {
        ...
        try {
            await logEvent({ userId: null, event: 'register', success: false, ip: req.ip, userAgent: req.headers['user-agent'] })
        } catch { }
        next(error)
    }
}
...
export const postLogout = async (req, res, next) => {
    try {
        ...
        res.clearCookie('refreshToken', { httpOnly: true, sameSite: 'lax', path: '/', secure: process.env.NODE_ENV === 'production' })
